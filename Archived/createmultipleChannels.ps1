...
$dataConfigs = Import-Excel -PATH "C:\eTaskAutomationTesting\ImportData.xlsx" -WorksheetName Config 
foreach ($dataConfig in $dataConfigs) {
    if ($dataConfig) {
    ...
    $dataExcel = Import-Excel -Path "C:\eTaskAutomationTesting\ImportData.xlsx" -WorksheetName "Data-Import"
    ...
    $resultSheet = $workbook.Worksheets.Item(3)
    ...
    foreach ($data in $dataExcel) {
        ...
        # assignedTo
        if ($data.channel -eq $dataConfig.channelName) {
            if ($data.EmailUser) {
            ...
            if ($data.projectName) {
                ...
                ForEach ($project in $myProjects) {
                    ...
                    if ($lengthprojectName -eq 0) {
                        $resultSheet.Cells.Item($countprojectName, 9) = $data.projectName
                        $resultSheet.Cells.Item($countprojectName, 9).Interior.ColorIndex = 3
                        $countprojectName++
                    }
                }
            }
