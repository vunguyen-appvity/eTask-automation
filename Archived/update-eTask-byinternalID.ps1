...
if ($dataConfig) {
    ...
    $dataExcel = Import-Excel -Path "C:\eTaskAutomationTesting\UpdateData.xlsx" 
                        ...
                        if ($data.phase) {
                            ...
                            if ($project.source -eq 'Appvity.eTask') {
                                ...
                                ForEach ($phase in $dataPhase.value) {
                                    ...
                                    if ($data.phase -eq $phase.phaseName) {
            ...
            if ($data.bucket) {
                ...
                # if $project.source not -eq 'Appvity.eTask'
                    ...
                    ForEach ($bucket in $dataBucket) {
                        if ($data.bucket -eq $bucket.displayName) {
                            $dataCreate.Add("bucketName", $data.bucket)
                            $dataCreate.Add("bucket", $bucket.id)
                            $haveBucket = $true
