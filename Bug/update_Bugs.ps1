...
if ($dataConfig) {
    ...
    $resultUpdateSheet.Cells.Item(1, 13) = 'severity'
    $resultUpdateSheet.Cells.Item(1, 14) = 'reportedBy'
    ...
    $b = Compare-Object -ReferenceObject $dataExcel -DifferenceObject $dataExcelCompare -Property name, priority, status, body, startDate, dueDate, EmailUser, phase, bucket, severity, reportedBy  -IncludeEqual -PassThru
    foreach ($r in $b) {
    ...
    foreach ($data in $data2) {
        ...
        if ($data.reportedBy) {
            ...
            $resultUpdateSheet.Cells.Item($countReported, 14) = $dirUser.displayName
            $countReported++
        }
        else {
            $dataCreate.Add("reportedBy", "")
            $resultUpdateSheet.Cells.Item($countReported, 14) = ""
            $resultUpdateSheet.Cells.Item($countReported, 14).Interior.ColorIndex = 15
            $countReported++
        }
            ...
            if ($mySeverity) {
                ForEach ($severity in $mySeverity.value) {
                    if ($data.severity -eq $severity.name) {
                        ...
                        $resultUpdateSheet.Cells.Item($countSeverity, 13) = $data.severity
                        $countSeverity++
                        break
                    }
                    $lengthSeverity--
                    if ($lengthSeverity -eq 0) {
                        $resultUpdateSheet.Cells.Item($countSeverity, 13) = $data.severity
                        $resultUpdateSheet.Cells.Item($countSeverity, 13).Interior.ColorIndex = 3
                        $countSeverity++  
                    }
                }
                # if ($statusSet) {
        ...
        try {
            $urlUpdate = 'https://' + $myDomain.TrimEnd('/') + '/odata/bugs(' + $data.ID + ')'
